...
function processLogo(img) {
  ...
  for (let i = 0; i < d.data.length; i += 4) {
    ...
    if (r > 220 && g > 220 && b > 220) {
      d.data[i+3] = 0;                          // white bg → transparent
    } else if (max < 120 && sat < 0.25) {
      d.data[i] = d.data[i+1] = d.data[i+2] = 255; // dark charcoal → white
    }
  }
...
document.querySelectorAll('.logo-full-img').forEach(img => {
  img.complete ? processLogo(img) : img.addEventListener('load', () => processLogo(img));
});
